...
final class TokenManager
{
    ...
    public function token(bool $fresh = false): string
    {
        ...
        if (!$apiKey) throw new ConfigurationException('PAYMOB_API_KEY is not configured.');
        $key = 'paymob:token:' . config('paymob.region') . ':' . hash('sha256', $apiKey);
        if ($fresh) $this->cache->forget($key);
    ...
    {
        $apiKey = config('paymob.keys.api');
        if ($apiKey) $this->cache->forget('paymob:token:' . config('paymob.region') . ':' . hash('sha256', $apiKey));
    }
    ...
    private function requestToken(string $apiKey): string
    {
        ...
        if (!$token) throw new ApiException($response->status(), $response->json(), 'Paymob authentication response did not contain a token.');
        return $token;
    }
